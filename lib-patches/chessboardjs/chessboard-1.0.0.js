...
;(function () {
  ...
  function constructor (containerElOrString, config) {
    ...
    function updateDraggedPiece (x, y) {
      ...
      // This is handled by unthrottled handlers for smoothness,
      // but we keep it here to ensure alignment when throttled logic runs.
      $draggedPiece.css({
        left: x - squareSize / 2,
        top: y - squareSize / 2
      })
    ...
    widget.destroy = function () {
      ...
      $container.unbind()
      // Unbind window events as well, specifically the ones added by this instance
      // A more robust way would be to namespace events, e.g., .on('mousemove.chessboard-' + uuid(), ...)
      // For now, a general unbind if no other chessboards are expected or if this is the only one.
      // $(window).unbind('mousemove mouseup touchmove touchend'); // This is too broad.
      // The current approach relies on $draggedPiece being removed, and handlers checking isDragging.
    }
    ...
    // For smooth visual updates, unthrottled
    function unthrottledVisualUpdateMousemove (evt) {
      if (isDragging && !$draggedPiece.is(':animated')) {
        $draggedPiece.css({
          left: evt.pageX - squareSize / 2,
          top: evt.pageY - squareSize / 2
        });
      }
    }

    // For smooth visual updates and immediate scroll prevention on touch
    function unthrottledVisualUpdateTouchmove (evt) {
      if (isDragging) {
        evt.preventDefault(); // Always prevent scroll if dragging
        if (!$draggedPiece.is(':animated')) { // Only update position if not animating
          $draggedPiece.css({
            left: evt.originalEvent.changedTouches[0].pageX - squareSize / 2,
            top: evt.originalEvent.changedTouches[0].pageY - squareSize / 2
          });
        }
      }
    }

    // Throttled version for logic
    ...
    function touchmoveWindow (evt) {
      ...
      // prevent screen from scrolling - MOVED to unthrottledVisualUpdateTouchmove
      // evt.preventDefault()
    ...
    function addEvents () {
      ...
      var $window = $(window)
      $window
          .on('mousemove', unthrottledVisualUpdateMousemove) // For smooth visuals
          .on('mousemove', throttledMousemoveWindow)         // For throttled logic
      ...
      if (isTouchDevice()) {
        ...
        $window
            .on('touchmove', unthrottledVisualUpdateTouchmove) // For smooth visuals & preventDefault
            .on('touchmove', throttledTouchmoveWindow)          // For throttled logic
